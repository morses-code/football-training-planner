...
{#if data.user}
	<div class="max-w-4xl">
		<h1 class="text-4xl font-bold text-slate-900 mb-4">My Profile</h1>
		<p class="text-lg text-slate-600 mb-6">Manage your account information.</p>
		...
		<div class="grid md:grid-cols-3 gap-6">
			...
			<div class="md:col-span-2">
				<div class="bg-white rounded-lg shadow p-6">
					{#if isEditing}
						<!-- Edit Mode -->
						<h2 class="text-2xl font-semibold text-slate-800 mb-4">Edit Profile</h2>

						<div class="flex flex-col mb-6">
							<label for="name" class="text-sm font-semibold text-slate-700 mb-2">Name</label>
							<input
								type="text"
								id="name"
						...
						<div class="flex flex-col mb-6">
							<p class="text-sm font-semibold text-slate-700 mb-3">Avatar</p>
							<div class="grid grid-cols-4 gap-3">
								{#each avatarOptions as avatar}
									<button
										...
										class:bg-blue-50={editData.avatar === avatar.id}
										class:border-slate-300={editData.avatar !== avatar.id}
									>
										<NavIcon
											path={avatar.path}
											class={`h-8 w-8 ${editData.avatar === avatar.id ? 'text-blue-600' : 'text-slate-600'}`}
										/>
						...
						<div class="flex gap-3">
							...
							<button
								...
								disabled={isSubmitting}
								class="flex-1 bg-slate-200 hover:bg-slate-300 text-slate-700 font-semibold py-3 px-6 rounded-lg transition-colors disabled:opacity-50"
							>
								Cancel
							</button>
						</div>
					{:else}
						...
						<div class="flex items-start gap-6 mb-6">
							<div class="h-20 w-20 text-slate-600 flex-shrink-0">
								<NavIcon
									path={avatarOptions.find((a) => a.id === data.user?.avatar)?.path ||
										avatarOptions[0].path}
									class="h-20 w-20"
								/>
							</div>
							<div class="flex-1">
								<h2 class="text-2xl font-semibold text-slate-800 mb-1">{data.user.name}</h2>
								<p class="text-slate-600 mb-4">{data.user.email}</p>
								<button
			...
			<div>
				<div class="bg-slate-100 rounded-lg p-6">
					<h2 class="text-xl font-semibold text-slate-800 mb-4">Account Info</h2>
					<div class="space-y-4">
						<div>
						...
						<div>
							...
							<p class="text-slate-600">Today</p>
						</div>
						<div>
							<p class="text-sm font-semibold text-slate-700 mb-1">Account Status</p>
